...
const debug = require('debug')('cnn-messaging:messenger');
const EventEmitter = require('events').EventEmitter;
const util = require('util');
const Rx = require('rxjs');
const Message = require('./message');
const SocketIORelay = require('./socket.io');

function Messenger(params) {
    if (!(this instanceof Messenger)) {
        return new Messenger(params);
    }
    ...
    if (params.http) {
        debug('got http instance, creating socket.io relay');
        SocketIORelay({
            server: params.http,
            messenger: this
        });
