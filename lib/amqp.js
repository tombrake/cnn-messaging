...
AmqpMessenger.prototype.stop = function stop() {
    return Messenger.prototype.stop.call(this)
        .then(() => {
            return this.channel.private.close();
        })
        .then(() => {
            return this.channel.work.close();
        })
        .then(() => {
...
AmqpMessenger.prototype.createNotificationObservable = function createNotificationObservable(topic) {
    ...
    if (this.observables.notification[topic]) {
        return this.observables.notification[topic];
    }

    this.observables.notification[topic] = Messenger.prototype.createNotificationObservable.call(this, topic)
        .then(() => {
            const queueparams = {
                durable: false,
                exclusive: true
        ...
        .then(() => {
            ...
            return new Rx.Observable.create((observer) => {
                ...
                return () => {
                    ...
                    delete this.observables.notification[topic];
                };
            });
        });
...
AmqpMessenger.prototype.createWorkObservable = function createWorkObservable(topic, queue) {
    // assert a private queue into existance and then bind it to the topic
    if (this.observables.work[topic]) {
        return this.observables.work[topic];
    }

    this.observables.work[topic] = Messenger.prototype.createWorkObservable.call(this, topic)
        .then(() => {
            const queueparams = {
                durable: true,
                exclusive: false
            };
            return this.channel.private.assertQueue(queue, queueparams);
        })
        .then((q) => {
        ...
        .then(() => {
            ...
            return new Rx.Observable.create((observer) => {
                ...
                return () => {
                    ...
                    delete this.observables.work[topic];
                };
            });
        });
