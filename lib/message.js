...
const messageSchema = Joi.object().keys({
    ...
    change: ({
        ...
        objectChanges: Joi.object().keys({
            addedProps: Joi.object(),
            removedProps: Joi.object(),
            updatedProps: Joi.object().keys({
                old: Joi.object(),
                new: Joi.object()
            }),
            incrementedProps: Joi.object(),
            decrementedProps: Joi.object()
        })
    }),
    event: Joi.object()
});
...
Message.fromKafka = function (m, topic, partition, consumer) {
    const messageString = m.message.value.toString('utf8');
    debug(`from kafka topic:${topic} partition:${partition} offset:${m.offset} message:${messageString}`);
...
Message.fromAmqp = function (rawMessage, channel) {
...
Message.prototype.toKafka = function toKafka() {
